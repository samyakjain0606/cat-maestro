...
export default function LoginPage() {
  ...
  const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Here you would typically call an API to perform the login
    ...
    if (response.ok) {
      // If login is successful, perform a full page reload to the home page
      window.location.href = '/';
    } else {
      // If there is an error, update the error state to display the message
      const result = await response.json();
      setError(result.message || 'Invalid username or password. Please try again or sign up if you haven\'t registered yet.');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        <div>
          ...
          >
            Log in to start your CAT journey
          </motion.p>
        </div>
        ...
        >
          <Link href="/signup" className="text-sm text-gray-300 hover:text-white transition-colors duration-200">
